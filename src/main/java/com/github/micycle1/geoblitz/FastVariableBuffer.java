...
/**
 * Identical to JTS' {@link org.locationtech.jts.operation.buffer.VariableBuffer
 * VariableBuffer} but uses
 * {@link com.github.micycle1.geoblitz.HilbertParallelPolygonUnion
 ...
 */
public class FastVariableBuffer {
	...
	public static Geometry buffer(Geometry line, double startDistance, double endDistance) {
		...
		FastVariableBuffer vb = new FastVariableBuffer(line, distance);
		return vb.getResult();
	}
	 ...
	 * @param distance the buffer distance for each vertex of the line
	 */
	public FastVariableBuffer(Geometry line, double[] distance) {
	...
	public Geometry getResult() {
		...
		Geometry buffer = HilbertParallelPolygonUnion.union(parts); // NOTE
	...
	private void addCap(Coordinate p, double r, Coordinate t1, Coordinate t2, CoordinateList coords) {
		...
		double angStart = angle(p, t1);
		double angEnd = angle(p, t2);
		if (angStart < angEnd)
			angStart += 2 * Math.PI;
		...
		double capSegLen = r * 2 * FastMath.sin(Math.PI / 4 / quadrantSegs);
	...
	private static LineSegment outerTangent(Coordinate c1, double r1, Coordinate c2, double r2) {
		...
		double y2 = c2.getY();
		// TODO: handle r1 == r2?
		double a3 = -FastMath.atan2(y2 - y1, x2 - x1);

		double dr = r2 - r1;
		double d = Math.sqrt((x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1));

		double a2 = FastMath.asin(dr / d);
		...
		double cos = FastMath.cos(aa);
		double sin = FastMath.sin(aa);
	...
	private static Coordinate projectPolar(Coordinate p, double r, double ang) {
		double x = p.getX() + r * snapTrig(FastMath.cos(ang));
		double y = p.getY() + r * snapTrig(FastMath.sin(ang));
