...
/**
 * Tests copied over from JTS.
 */
public class FastVariableBufferTests {
	...
	public void testIssue998_Spike() {
		checkBuffer("LINESTRING (0.024520295 69.50077743, 0.000508719 74.50086084, 0 76.39546845)", new double[] { 6.47, 6.9, 7 },
	...
	private void checkBuffer(String wkt, double startDist, double endDist, String wktExpected) {
		...
		Geometry result = FastVariableBuffer.buffer(geom, startDist, endDist);
		checkBuffer(result, wktExpected);
	}

	private void checkBuffer(String wkt, double[] dist, String wktExpected) {
		Geometry geom = read(wkt);
		Geometry result = FastVariableBuffer.buffer(geom, dist);
		checkBuffer(result, wktExpected);
	}
	...
	private void checkBuffer(Geometry actual, String wktExpected) {
		...
		Geometry normActual = (Geometry) actual.copy();
		Geometry normExpected = (Geometry) expected.copy();
		normActual.normalize();
		normExpected.normalize();

		boolean equal = normExpected.equalsExact(normActual, DEFAULT_TOLERANCE);
		if (!equal) {
			fail("Geometries differ.\nExpected: " + normExpected.toText() + "\nActual:   " + normActual.toText());
		}
	}
	...
	private Geometry read(String wkt) {
		try {
			...
			fail("Failed to parse WKT: " + e.getMessage());
